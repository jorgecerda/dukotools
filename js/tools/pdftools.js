...
export default {
    ...
    setupTabs() {
        ...
        btnC.addEventListener('click', () => {
            btnC.classList.add('active');    panC.classList.add('active');
            btnX.classList.remove('active'); panX.classList.remove('active');
        });
        btnX.addEventListener('click', () => {
            btnX.classList.add('active');    panX.classList.add('active');
            btnC.classList.remove('active'); panC.classList.remove('active');
        });
    },
    ...
    setupQualityPills() {
        document.querySelectorAll('[data-quality]').forEach(pill => {
            pill.addEventListener('click', () => {
                document.querySelectorAll('[data-quality]').forEach(p => p.classList.remove('active'));
    ...
    setupCompressTab() {
        ...
        this.bindDropzone(dropzone, fileInput, files => this.addToQueue(files, 'compress'));
        document.getElementById('compress-clear-btn').addEventListener('click', () => {
            this.compressQueue = [];
            this.renderQueue('compress');
        });
        document.getElementById('compress-run-btn').addEventListener('click', () => this.runCompress());
    },
    ...
    setupConvertTab() {
        ...
        this.bindDropzone(dropzone, fileInput, files => this.addToQueue(files, 'convert'));
        document.getElementById('convert-clear-btn').addEventListener('click', () => {
            this.convertQueue = [];
            this.renderQueue('convert');
        });
        document.getElementById('convert-run-btn').addEventListener('click', () => this.runConvert());
    },
    ...
    renderQueue(mode) {
        ...
        listEl.innerHTML = queue.map(item => {
            ...
            const progressStyle = (item.status === 'processing')
        ...
        listEl.querySelectorAll('.btn-download-result').forEach(btn => {
            btn.addEventListener('click', () => this.downloadItem(btn.dataset.id, btn.dataset.mode));
        });
        // Remove listeners
        listEl.querySelectorAll('.btn-remove-file').forEach(btn => {
            btn.addEventListener('click', () => {
                const m = btn.dataset.mode;
                if (m === 'compress') this.compressQueue = this.compressQueue.filter(i => i.id !== btn.dataset.id);
                else this.convertQueue = this.convertQueue.filter(i => i.id !== btn.dataset.id);
                this.renderQueue(m);
            });
        });
    },
    ...
    async runCompress() {
        ...
        if (!pending.length) { alert('No pending files to compress.'); return; }

        const preset = this.QUALITY_PRESETS[this.selectedQuality] || this.QUALITY_PRESETS.balanced;
        ...
        for (const item of pending) {
            ...
            try {
                ...
                const pdfJsDoc = await this.pdfjsLib.getDocument({ data: arrayBuffer }).promise;
                ...
                for (let pageNum = 1; pageNum <= pageCount; pageNum++) {
                    ...
                    const jpegBytes = await this._canvasToJpegBytes(canvas, preset.jpegQuality);
    ...
    _canvasToJpegBytes(canvas, quality) {
        return new Promise((resolve, reject) => {
            canvas.toBlob(blob => {
                if (!blob) { reject(new Error('Canvas to JPEG failed')); return; }
    ...
    async runConvert() {
        ...
        for (const item of pending) {
            ...
            try {
                const outputName = item.name.replace(/\.pdf$/i, `.${this.selectedFormat}`);
                const blob = await this._cloudConvertFile(item.file, this.selectedFormat, (pct) => {
                    item.progress = pct;
    ...
    async _cloudConvertFile(file, outputFormat, onProgress) {
        ...
        if (!startRes.ok) {
            ...
            throw new Error(err.error || `Failed to start conversion (${startRes.status})`);
        }
        ...
        for (let attempt = 0; attempt < 90; attempt++) {
            ...
            if (data.status === 'error') throw new Error(data.message || 'Conversion failed on CloudConvert.');
            if (data.status === 'finished') { downloadUrl = data.downloadUrl; break; }
        ...
        if (!downloadUrl) throw new Error('Conversion timed out after 3 minutes. Please try again.');
        onProgress(90);
    ...
    downloadItem(id, mode) {
        ...
        link.href = url; link.download = item.result.outputName;
    ...
    async loadPdfLib() {
        ...
        await new Promise((resolve, reject) => {
            if (window.PDFLib) { this.PDFLib = window.PDFLib; resolve(); return; }
            const s = document.createElement('script');
            s.src = 'https://unpkg.com/pdf-lib@1.17.1/dist/pdf-lib.min.js';
            s.onload  = () => { this.PDFLib = window.PDFLib; resolve(); };
    ...
    async loadPdfJs() {
        ...
        await new Promise((resolve, reject) => {
            if (window.pdfjsLib) { this.pdfjsLib = window.pdfjsLib; resolve(); return; }
            const s = document.createElement('script');
